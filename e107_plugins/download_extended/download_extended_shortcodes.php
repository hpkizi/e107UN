 ...
 * download_shortcodes
 */
class download_extended_shortcodes extends e_shortcode
{
	public $qry = array();
	public $dlsubrow;
	public $dlsubsubrow;
    ...
    public function sc_jmdownload_period($parm='', $mode='')
	{
    ...
    //see meta description in download_class handler
    function sc_jmdownload_meta_description($parm = '')
	{
		$maxlen = ($parm ? intval($parm) : 287);
 	    //$text = ($this->var['download_description'] ? e107::getParser()->toHTML($this->var['download_description'], true, 'DESCRIPTION') : "");
 
        $metaDescription                = e107::getParser()->toHTML($this->var['download_description'],true);
        $metaDescription                = str_replace("\n", ' ', $metaDescription ); // clean-out line-breaks.
 
		if ($maxlen)
		{
			$metaDescription    	= e107::getParser()->text_truncate($metaDescription, $maxlen); // + '...'
		}
		else
		{
